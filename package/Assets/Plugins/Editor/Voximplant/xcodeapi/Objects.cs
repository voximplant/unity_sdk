...
namespace Xcodeapi.PBX
{
    ...
    internal class PBXBuildFileData : PBXObjectData
    {
        ...
        PBXElementArray EnsureElementArrayWithinDictionary(PBXElementDict dict, string name)
        {
            if (dict == null)
                dict = m_Properties.CreateDict("settings");
            PBXElementArray element = null;
            if (dict.Contains(name))
                element = dict[name].AsArray();
            else
                element = dict.CreateArray(name);

            return element;
        }

        bool ElementArrayContainsString(PBXElementArray array, string element)
        {
            foreach (var value in array.values)
            {
                if (value is PBXElementString && value.AsString() == element)
                    return true;
            }
            return false;
        }
        ...
        public override void UpdateProps()
        {
            ...
            if (weak)
            {
                var attrs = EnsureElementArrayWithinDictionary(settings, "ATTRIBUTES");
                var weakElementName = "Weak";
                if (!ElementArrayContainsString(attrs, weakElementName))
                    attrs.AddString(weakElementName);
            }
            else
            {
            ...
            {
                var attrs = EnsureElementArrayWithinDictionary(settings, "ATTRIBUTES");
                var signElementName = "CodeSignOnCopy";
                if (!ElementArrayContainsString(attrs, signElementName))
                    attrs.AddString(signElementName);
            }
            else
            {
                if (settings != null && settings.Contains("ATTRIBUTES"))
                {
                    var attrs = settings["ATTRIBUTES"].AsArray();
                    attrs.values.RemoveAll(el => (el is PBXElementString && el.AsString() == "CodeSignOnCopy"));
                    if (attrs.values.Count == 0)
                        settings.Remove("ATTRIBUTES");
                }
            }
            ...
            if (m_Properties.Contains("settings"))
            {
                ...
                if (dict.Contains("ATTRIBUTES"))
                {
                    ...
                    foreach (var value in attrs.values)
                    {
                        ...
                        if (value.AsString() == "Weak")
                            weak = true;

                        if (value.AsString() == "CodeSignOnCopy")
                            codeSignOnCopy = true;
                    }
                }
    ...
    internal class PBXCopyFilesBuildPhaseData : FileGUIDListBase
    {
        ...
        public string name;
        public SubfolderSpec subfolderSpec;

        // Taken from https://github.com/facebook/buck/blob/master/src/com/facebook/buck/apple/xcode/xcodeproj/PBXCopyFilesBuildPhase.java#L26
        ...
        /// <param name="subfolderSpec"></param>
        /// <returns></returns>
        public static PBXCopyFilesBuildPhaseData Create(string name, SubfolderSpec subfolderSpec)
        {
            ...
            res.SetPropertyString("dstSubfolderSpec", ((int)subfolderSpec).ToString());
            res.subfolderSpec = subfolderSpec;
            res.files = new List<string>();
            res.SetPropertyString("runOnlyForDeploymentPostprocessing", "0");
            res.name = name;
            return res;
        ...
        public override void UpdateProps()
        {
            SetPropertyList("files", files);
            SetPropertyString("name", name);
            SetPropertyString("dstSubfolderSpec", ((int)subfolderSpec).ToString());
        }
        public override void UpdateVars()
        {
            files = GetPropertyList("files");
            ...
            var propertyValue = GetPropertyString("dstSubfolderSpec");
            int subfolderSpecInt = 0;
            if (Int32.TryParse(propertyValue, out subfolderSpecInt)) {
                subfolderSpec = (SubfolderSpec) subfolderSpecInt;
            }
        }
    }
