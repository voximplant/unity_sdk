...
/*!
 Callback for preprocessing audio data
 @param audio10ms pointer to PCM audio data buffer, audio data must be modified if needed synhroniously in place
 @param length buffer lenght in words
 @param samplingFreq sampleing frequence
 @param isStereo stereo or mono
 */
@optional
- (void) onPreprocessAudioPacket:(int16_t*)audio10ms lenght:(size_t)length samplingFreq:(int)samplingFreq isStereo:(bool)isStereo;
