...
public class VideoFrame {
    ...
    public static VideoFrame fromI420Frame(final VideoRenderer.I420Frame frame) {
        final VideoFrame.Buffer buffer;
        if (frame.yuvFrame) {
            buffer = new I420BufferImpl(frame.width, frame.height, frame.yuvPlanes[0], frame.yuvStrides[0], frame.yuvPlanes[1],
            ...
            buffer = new TextureBufferImpl(frame.width, frame.height, VideoFrame.TextureBuffer.Type.OES, frame.textureId,
                    RendererCommon.convertMatrixToAndroidGraphicsMatrix(frame.samplingMatrix),
                    null /* surfaceTextureHelper */, new Runnable() {
                @Override
                public void run() {
                    VideoRenderer.renderFrameDone(frame);
                }
        ...
        return new VideoFrame(buffer, frame.rotationDegree, 0 /* timestampNs */);
    }
