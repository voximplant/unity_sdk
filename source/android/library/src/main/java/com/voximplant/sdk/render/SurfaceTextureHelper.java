...
public class SurfaceTextureHelper {
    ...
    private SurfaceTextureHelper(EglBase.Context sharedContext, Handler handler) {
        ...
        try {
            ...
            eglBase = EglBase.createAdaptive(sharedContext, EglBase.CONFIG_PIXEL_BUFFER);
        } catch (RuntimeException e) {
    ...
    public VideoFrame.TextureBuffer createTextureBuffer(int width, int height, Matrix transformMatrix) {
        return new TextureBufferImpl(
                width, height, VideoFrame.TextureBuffer.Type.OES, oesTextureId, transformMatrix, this, new Runnable() {
            @Override
            public void run() {
                returnTextureFrame();
            }
